...
const Footer: React.FC = () => {
  return (
    <footer className="bg-space-dark text-brand-cream-light py-12 border-t border-brand-green/10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
          ...
          <div className="mb-6 md:mb-0">
            ...
            <ul className="space-y-2">
              {navLinks.filter(link => !('sublinks' in link)).map((link) => (
                <li key={link.name}>
                  <Link to={link.href} className="hover:text-brand-green transition-colors duration-300">{link.name}</Link>
                </li>
              ))}
            </ul>
          </div>
          ...
          <div className="mb-6 md:mb-0">
            ...
            <ul className="space-y-2">
              {(navLinks.find(link => link.name === 'Portals') as any)?.sublinks?.map((sublink: any) => (
                <li key={sublink.name}>
                  <Link to={sublink.page} className="hover:text-brand-green transition-colors duration-300">{sublink.name}</Link>
                </li>
              ))}
            </ul>
          </div>
